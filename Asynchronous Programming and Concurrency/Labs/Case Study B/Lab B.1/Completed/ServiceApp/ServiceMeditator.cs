...
namespace ServiceApp
{
    class ServiceMeditator : IAsyncServiceMediator
    {
        ...
        public async Task OnNoteUpdatedAsync(IClientAsyncCallbackContract originatingClient, int serial, string note)
        {
            string message = $"{serial}\t{note}\tFrom: {originatingClient.GetHashCode()}";
            ...
            {
                callbackTasks = _clients
                    .Where( client => client != originatingClient)
                    .Select(client => client.NoteUpdatedAsync(serial,note))
                    .ToArray()
                    ;
            }
